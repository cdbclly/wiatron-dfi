...
@Component({
  selector: 'app-test-time-chart',
  templateUrl: './test-time-chart.component.html',
  styleUrls: ['./test-time-chart.component.scss']
})
export class TestTimeChartComponent implements OnInit, OnDestroy {
  ...
  async queryOp (options, type?) {
    ...
    this.data_arr, this.cur_model, this.cur_item, this.cur_upn,  this.datefrom, this.dateto, false);
    this.timer = setInterval(async() => {
      ...
      this.getTestTimeScatter('test-time', this.dataX_array, this.data_testTime, this.testT_itemArray, this.data_map, this.limit_arr,
    this.data_arr, this.cur_model, this.cur_item, this.cur_upn, this.datefrom, this.dateto, true);
  }
  ...
  async getTestTimeScatter(type: string, dataX_array: string[], data_testTime: any[], testT_itemArray: TestTimes[],
    data_map: Map<string, {}[]>, limit_arr: Limit, data_arr: number[], cur_model, cur_item , cur_upn, datefrom, dateto, dateTag) {
    ...
    if (datefrom !== undefined && dateto !== undefined) {
      ...
      date_range = `"range": {
        "stopdate": {
          "lte": "now",
          "gte": ${cur_date.getTime() - 18000000}
        }
      }`;
      size = `"size": 10`;
      extended_bounds = `"extended_bounds": {
        "min": ${cur_date.getTime() - 18000000},
        "max": "now"
      }`;
    }
    let querys;
    this.modelArr = [];
    querys = this.esService.getTestTimeQuerys(this.cur_site.toLowerCase(),
    this.cur_line.toUpperCase(),
    this.cur_mdname.toUpperCase(), this.cur_plant.toUpperCase(),
    this.cur_model, cur_item, cur_upn, date_range, size, extended_bounds);
    console.log('querys 请求的参数', querys);
    ...
    //  await this.esService.postData(esURL, querys).subscribe(data => {
      ...
      try {
        ...
        console.log('请求得到的数据 == \n', data);
        if (data['aggregations']['sales_over_time']['buckets'].length > 500) {
      ...
      data['aggregations']['sales_over_time']['buckets'][i]['TopHits']['hits']['hits'].forEach(element => {
        ...
        element._source.detail.forEach(detail => {
          ...
          const det = {
            ...
            // 添加开始结束时间
            startdate: detail.startdate < 0 ? '' : detail.startdate,
            stopdate: detail.stopdate,
            testcycletime: detail.testcycletime,
            mean_testcycletime: element._source.mean_testcycletime
          };
          map_value.push(det);
    ...
    // if (this.limit_arr) {
      dataX_array.push('');
      this.options_scatter(this.limit_arr, dataX_array, dateTag);
    // }
    // });
    this.modelArr = Array.from(new Set(this.modelArr)); // 去重
    console.log(this.modelArr);
  ...
  async options_scatter(limit_arr, dataX_array, dateTag) {
    let max;
    let min;
    let mean;
      ...
      series: [{
          ...
          markLine : {
           ...
           [
            ...
            { name : 'lower', xAxis: dataX_array.length - 1, yAxis: this.limit_arr === undefined || this.limit_arr.lowlimit === undefined ? 0 : this.limit_arr.lowlimit}
           ]]
         }
      }]
  };
    ...
    if (data) {
      const downloadDatas = data.map (res => {
        return {
          ...
          '治具編號': res['fix'],
          '序列號': res['unitNum'],
          // '測試時間': this.datePipe.transform(res['test_time'], 'yyyy-MM-dd HH:mm:ss'),
          // '測試值': res['test_long'],
          '開始測試時間': this.datePipe.transform(res['startdate'], 'yyyy-MM-dd HH:mm:ss'),
          '結束測試時間': this.datePipe.transform(res['stopdate'], 'yyyy-MM-dd HH:mm:ss'),
          '測試總時間/s': res['testcycletime'],
          '上限': res['upper'],
          '中心值': res['mean'],
          '下限': res['lower']
        };
